...
from quiz_api.models.multiple_choice_option import (
...
@multiple_choice_option_router.post(
    "",
    response_model=MultipleChoiceOptionCreate,
...
async def update_multiple_choice_option(
    ...
    with Session(db_engine) as session:
        ...
        db_option = MultipleChoiceOption.model_validate(option, db_option)
        session.add(db_option)
        session.commit()
